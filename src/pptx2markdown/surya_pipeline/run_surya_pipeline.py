...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Working directory for intermediate outputs (target_pdf, output, ...). "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: <work-dir>/target_pptx"
        ),
    )
    parser.add_argument(
        "--target-slides-dir",
        default=None,
        help="Directory for extracted pptx bundles. Default: <work-dir>/target_slides",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    run_cmd(
        [
            sys.executable,
            "-m",
            "pptx2markdown.surya_pipeline.convert_pptx_to_pdf",
            "--input-dir",
            str(staged_pptx_dir),
            "--output-dir",
    ...
    try:
        ...
        raise RuntimeError(
            "Surya is not installed. --reading-order surya requires the optional extra: "
            'pip install "pptx2markdown[surya]"'
        ) from e
        ...
        if args.force or not normalized_json.exists():
            ...
            run_cmd(
                [
                    sys.executable,
                    "-m",
                    "pptx2markdown.surya_pipeline.normalize_surya_results",
                    "--layout-json",
                    str(layout_json),
                    *ppt_locator_args,
                    "--output-json",
        ...
        if args.force or not reordered_xmls:
            ...
            run_cmd(
                [
                    sys.executable,
                    "-m",
                    "pptx2markdown.surya_pipeline.build_structure_ready_from_normalized",
                    "--normalized-json",
                    str(normalized_json),
                    *ppt_locator_args,
                    "--output-dir",
